	...
	int update = jdbcTemplate.update(
			new PreparedStatementCreator() {
					...
					PreparedStatement ps =
									...
									ps.setString(3, expensesBean.getDiscription());
									ps.setString(4, expensesBean.getAmount());
									ps.setTimestamp(5, new java.sql.Timestamp(expensesBean.getDairydate().getTime()));
	...
	return insert;
	 ...
	 public List<ExpensesBean> getExpensesBeanAll(String date) throws ParseException {		 
		 ...
		 if (StringUtils.isNotBlank(date)) {
			 ...
			 buffer.append("Date(ld.dairydate)= Date('" + new java.sql.Timestamp(date1.getTime())+"')");
			}else {
				
				buffer.append(" Date(ld.dairydate)=CURDATE()");
			}
